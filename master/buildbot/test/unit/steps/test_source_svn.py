...
class TestSVN(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_mode_incremental_timeout(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', timeout=1, command=['svn', '--version']).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectStat(file='wkdir/.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='wkdir',
            ...
            ExpectShell(workdir='wkdir', timeout=1, command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
        self.expect_outcome(result=SUCCESS)
    ...
    def test_mode_incremental_repourl_not_updatable(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', command=['svn', '--version']).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectStat(file='wkdir/.svn', log_environ=True).exit(1),
    ...
    def test_mode_incremental_win32path(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectStat(file=r'wkdir\.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='wkdir',
                command=[
                    'svn',
    ...
    def test_mode_full_fresh_keep_on_purge(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectRmdir(
                dir=['wkdir/svn_external_path/unversioned_file2_uniçode'],
                log_environ=True,
                timeout=1200,
            ).exit(0),
            ExpectShell(
    ...
    def test_mode_full_clean_old_rmdir(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectRmdir(
                dir='wkdir/svn_external_path/unversioned_file1', log_environ=True, timeout=1200
            ).exit(0),
            ExpectRmdir(
                dir='wkdir/svn_external_path/unversioned_file2_uniçode',
                log_environ=True,
    ...
    def test_mode_full_copy(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectRmdir(dir='wkdir', log_environ=True, timeout=1200).exit(0),
            ExpectStat(file='source/app/.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='source/app',
                command=['svn', 'info', '--xml', '--non-interactive', '--no-auth-cache'],
            )
            ...
            ExpectCpdir(fromdir='source/app', todir='wkdir', log_environ=True).exit(0),
            ExpectShell(workdir='wkdir', command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
    ...
    def test_mode_full_copy_given_revision(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectRmdir(dir='wkdir', log_environ=True, timeout=1200).exit(0),
            ExpectStat(file='source/.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='source',
                command=['svn', 'info', '--xml', '--non-interactive', '--no-auth-cache'],
            )
            ...
            ExpectCpdir(fromdir='source', todir='wkdir', log_environ=True).exit(0),
            ExpectShell(workdir='wkdir', command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
    ...
    def test_mode_full_export(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectShell(
                workdir='source', command=['svn', 'update', '--non-interactive', '--no-auth-cache']
            ).exit(0),
            ExpectShell(workdir='', command=['svn', 'export', 'source', 'wkdir']).exit(0),
            ExpectShell(workdir='source', command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
        self.expect_outcome(result=SUCCESS)
    ...
    def test_mode_full_export_timeout(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', timeout=1, command=['svn', '--version']).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectRmdir(dir='wkdir', log_environ=True, timeout=1).exit(0),
            ExpectStat(file='source/.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='source',
                timeout=1,
            ...
            ExpectShell(workdir='', timeout=1, command=['svn', 'export', 'source', 'wkdir']).exit(
                0
            ),
            ExpectShell(workdir='source', timeout=1, command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
        self.expect_outcome(result=SUCCESS)
    ...
    def test_mode_full_export_given_revision(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir='', command=['svn', 'export', '--revision', '100', 'source', 'wkdir']
            ).exit(0),
            ExpectShell(workdir='source', command=['svn', 'info', '--xml'])
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
    ...
    def test_mode_incremental_with_env(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', command=['svn', '--version'], env={'abc': '123'}).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectStat(file='wkdir/.svn', log_environ=True).exit(0),
            ExpectShell(
                workdir='wkdir',
            ...
            ExpectShell(workdir='wkdir', command=['svn', 'info', '--xml'], env={'abc': '123'})
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
        self.expect_outcome(result=SUCCESS)
    ...
    def test_mode_incremental_log_environ(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', command=['svn', '--version'], log_environ=False).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=False).exit(1),
            ExpectStat(file='wkdir/.svn', log_environ=False).exit(0),
            ExpectShell(
                workdir='wkdir',
                command=[
                    'svn',
            ...
            ExpectShell(workdir='wkdir', command=['svn', 'info', '--xml'], log_environ=False)
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
        self.expect_outcome(result=SUCCESS)
    ...
    def test_bogus_svnversion(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir='wkdir', command=['svn', 'info', '--xml']).stdout('1x0y0').exit(0),
        )
        self.expect_outcome(result=FAILURE)
        return self.run_step()
    ...
    def test_rmdir_fails_clobber(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', command=['svn', '--version']).exit(0),
            ExpectStat(file='wkdir/.buildbot-patched', log_environ=True).exit(1),
            ExpectRmdir(dir='wkdir', log_environ=True, timeout=1200).exit(1),
        )
        self.expect_outcome(result=FAILURE)
        return self.run_step()
    ...
    def test_cpdir_fails_copy(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ...
            ExpectCpdir(fromdir='source', todir='wkdir', log_environ=True).exit(1),
        )
        self.expect_outcome(result=FAILURE)
        return self.run_step()
    ...
    def test_worker_connection_lost(self) -> defer.Deferred[None]:
        ...
        self.expect_commands(
            ExpectShell(workdir='wkdir', command=['svn', '--version']).error(error.ConnectionLost())
        )
